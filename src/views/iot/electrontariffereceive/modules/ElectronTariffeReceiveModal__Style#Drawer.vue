  ...
  <a-drawer
    ...
    <a-spin :spinning="confirmLoading">
      <a-form :form="form">
        ...
        <a-form-item label="(1:申请成功，2:充值成功，3:充值失败)" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <a-input-number v-decorator="[ 'orderStatus', validatorRules.orderStatus]" placeholder="请输入(1:申请成功，2:充值成功，3:充值失败)" style="width: 100%"/>
        </a-form-item>
...
<script>
  ...
  export default {
    name: "ElectronTariffeReceiveModal",
    components: { 
      JDate,
    },
    data () {
      return {
        ...
        url: {
          add: "/electrontariffereceive/electronTariffeReceive/add",
          edit: "/electrontariffereceive/electronTariffeReceive/edit",
        }
      }
    },
    created () {
    ...
    methods: {
      ...
      edit (record) {
        ...
        this.visible = true;
        this.$nextTick(() => {
          this.form.setFieldsValue(pick(this.model,'phone','orderStatus','createTime','updateTime'))
        })
      },
      close () {
        this.$emit('close');
      ...
      popupCallback(row){
        this.form.setFieldsValue(pick(row,'phone','orderStatus','createTime','updateTime'))
      }
