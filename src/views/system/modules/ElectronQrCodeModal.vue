  ...
  <!--<a-modal-->
   ...
   <!--&gt;-->
    <!--<div style="width: 100%;height: 100%;position: relative;padding: 8px;border: 0px solid #d9d9d9;border-radius: 4px;">-->
      <!--&lt;!&ndash;<img style="width: 100%;" :src="imgUrl"  :preview="dataSource[0].key">&ndash;&gt;-->
  ...
  export default {
    ...
    data () {
      return {
        ...
        visible: false,
        url: {
          getQrcode:"/sys/telecomAgent/generaQrCode", // 引入生成添加用户情况下的url
        },
      }
    },
    created () {
    ...
    methods: {
      show(id,ifvoice) {
        getAction(this.url.getQrcode+"/"+id+"/"+ifvoice, null).then((res) => {
          if (res.success) {
