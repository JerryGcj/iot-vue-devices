        ...
        <a-row class="form-row" :gutter="0">
          ...
          <a-col :lg="8">
            <a-form-item
              ...
              :wrapperCol="wrapperCol"
              label="套餐">
              <!--<a-select v-decorator="[ 'formData.packageId', {}]"
                        placeholder="套餐">
                <a-select-option v-for="d in packageData" :key="d.value" :value="d.value">{{d.text}}</a-select-option>
              </a-select>-->
              <j-dict-select-tag v-model="formData.packageId" placeholder="请选择套餐" dictCode="iot_standard_cost,package_name,id,statue = '0' and operator_type='2'"/>
            </a-form-item>
          </a-col>
  ...
  export default {
    ...
    methods: {
          ...
          if (!err) {
            ...
            this.$refs.editableTable1.getValues((error, values) => {
              ...
              if (error === 0) {
                ...
                let httpurl = '/unicomrefcardcost/unicomRefCardCost/updateAndAdd';
                let method = 'post';
      ...
      queryDiscountPackage(){
        ...
        var that = this;
        queryDiscountPackage({operatorType:"2"}).then((res)=>{
          if(res.success){
            that.packageData = [];
            let treeList = res.result
            for(let a=0;a<treeList.length;a++){
              let temp = treeList[a];
              that.packageData.push({
              ...
              for(let a=0;a<treeList.length;a++){
                ...
                if(inclusionType == '0'){
                  ...
                  if(packageId == temp.id){
                    ...
                    queryStandardcostById({id: temp.id}).then((res)=>{
      ...
      handleDelete(props) {
        ...
        for (let i = 0; i < packageValue.length; i++) {
          if(rowId == packageValue[i].id){
            let httpurl = '/unicomrefcardcost/unicomRefCardCost/delete';
