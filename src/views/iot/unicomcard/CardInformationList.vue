        ...
        <a-row :gutter="24">
          ...
          <a-col :md="8" :sm="10">
            <a-form-item label="ICCID">
              <a-input placeholder="开始ICCID" class="query-group-cust" v-model="queryParam.virtualIccidStart"></a-input>
              <span class="query-group-split-cust"></span>
              <a-input placeholder="结束ICCID" class="query-group-cust" v-model="queryParam.virtualIccidEnd"></a-input>
            </a-form-item>
          </a-col>
          <a-col :md="8" :sm="10">
          ...
          <a-col :md="4" :sm="6">
            <a-form-item label="卡状态">
              <j-dict-select-tag v-model="queryParam.cardState" placeholder="请选择运营商" dict-code="sim_status"></j-dict-select-tag>
<!--              <a-select v-model="queryParam.cardState" placeholder="请选择">-->
<!--                <a-select-option value="0">可测试</a-select-option>-->
...
<!--                <a-select-option value="4">已失败</a-select-option>-->
<!--                <a-select-option value="5">已清除</a-select-option>-->
<!--                <a-select-option value="6">已更换</a-select-option>-->
<!--                <a-select-option value="7">库存</a-select-option>-->
<!--                <a-select-option value="8">开始</a-select-option>-->
<!--                <a-select-option value="9">机卡分离</a-select-option>-->
...
<!--              </a-select>-->
    ...
    <div class="table-operator">
      ...
      <a-button type="primary" v-has="'cardInformationList:flowPool'" icon="" @click="showFlowPool">流量池统计</a-button>
      <a-button type="primary" v-has="'cardInformationList:communicationPlan'" icon="" @click="speedLimitChange">设备限速</a-button>
      <a-button type="primary" v-has="'cardInformationList:handleExportXls'" icon="download" @click="handleExportXles('联通卡信息表')" :disabled="isDisable">导出</a-button>
    ...
    <div>
      ...
      <a-table
        ...
        <span slot="action" slot-scope="text, record">
          ...
          <a v-has="'cardInformationList:dayViewPackageList'" @click="cardUsage(record)"><a-divider type="vertical" />日用量</a>
          <a-dropdown>
            <a v-has="'unicomCardInformation:more'" class="ant-dropdown-link">
      ...
      return {
        ...
        columns: [
           ...
           },
          {
            title: '自定义ICCID',
            align:"center",
            dataIndex: 'customIccid', width: 250
          },
          /*{
            title: '运营商类型 ',
            align:"center",
          ...
          },
          {
            title: '激活时间',
            align:"center",
            dataIndex: 'firstPurchaseTime', width: 200
          },
          {
            title: '用量倍数',
            align:"center",
          ...
          },
          {
		    ...
		    url: {
          list: "/unicomcardinformation/unicomCardInformation/listImprove",
    ...
    methods: {
      ...
      handleCardDistribution: function () {
        ...
        if((null==this.queryParam.customerId||'undefined'==this.queryParam.customerId||""==this.queryParam.customerId)&&
          ...
          (null==this.queryParam.virtualIccidStart||'undefined'==this.queryParam.virtualIccidStart||""==this.queryParam.virtualIccidStart)&&
          (null==this.queryParam.virtualIccidEnd||'undefined'==this.queryParam.virtualIccidEnd||""==this.queryParam.virtualIccidEnd)&&
      ...
      handleCardDistributionPackageOne: function (record) {
        ...
        getUnicomRefCardCostByCardId({iccid:record.iccid}).then((res)=>{
          if (res.success) {
      ...
      showFlowPool: function () {
        getAction('/unicomflowpool/iotUnicomFlowPool/list', {}).then((res) => {
          if (res.success) {
            this.$refs.CardInformationPriority.add();
            this.$refs.CardInformationPriority.title = "流量池详情";
            this.$refs.CardInformationPriority.disableSubmit = true;
          } else {
            this.$message.warning(res.message);
          }
        })
      },
      handleExportXles(fileName){
        this.isDisable = true;
