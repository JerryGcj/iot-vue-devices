<template>
  <a-card :bordered="false">
        ...
        <a-row :gutter="24">
          ...
          <!--<a-col :md="6" :sm="8">-->
            <!--<a-form-item label="运营商">-->
              ...
              <!--&lt;!&ndash;<a-select :dropdownStyle="{ maxHeight: '200px', overflow: 'auto' }"&ndash;&gt;-->
                        <!--&lt;!&ndash;v-model="queryParam.operatorId"&ndash;&gt;-->
                        <!--&lt;!&ndash;placeholder="请选择运营商">&ndash;&gt;-->
                <!--&lt;!&ndash;<a-select-option v-for="d in operatorData" :key="d.value" :value="d.value">{{d.text}}</a-select-option>&ndash;&gt;-->
    ...
    <div class="table-operator">
      ...
      <a-button type="primary" v-has="'cardInformationList:realNameReply'" icon="" @click="realNameReply">实名复机</a-button>
      ...
      <a-button type="primary" v-has="'cardInformationList:handleExportXls'"icon="download" @click="handleExportXls('移动卡信息表')">导出</a-button>
    ...
    <div>
      ...
      <a-table
        ...
        :scroll="{ x: 1800 }">
        <span slot="iccid" slot-scope="iccid">
            <a v-text="iccid" @click="refreshStatus(iccid)"></a>
        </span>
        <span slot="action" slot-scope="text, record">
          <a v-has="'cardInformationList:viewPackageList'" @click="handleDetails(record)">套餐列表</a>
          <a v-has="'cardInformationList:dayViewPackageList'" @click="handleCardDailyDose(record)"><a-divider type="vertical" />日用量</a>
          <a-dropdown>
...
<script>
      ...
      return {
        ...
        columns: [
           ...
           },
          {
            title: '自定义ICCID',
            align:"center",
            dataIndex: 'virtualIccid', width: 250
          },
		      {
            title: 'MSISDN',
            align:"center",
          ...
          },
          {
            title: '实名状态',
            align:"center",
            dataIndex: 'isRealNameAuthentication',
  ...
  computed: {
    ...
    methods: {
      refreshStatus(id){
        getAction('/cardinformation/cardInformation/getStopReason?iccid='+id,null).then((res) => {
      ...
      //实名复机
      realNameReply: function () {
        if (this.selectedRowKeys.length != 1) {
          this.$message.warning('请选择一条记录！');
          return;
        }else {
          getAction('/cardinformation/cardInformation/realNameReply', {id:this.selectedRowKeys[0]}).then((res) => {
            if(res.success){
              this.$message.success(res.message);
              this.onClearSelected();
              this.loadData();
