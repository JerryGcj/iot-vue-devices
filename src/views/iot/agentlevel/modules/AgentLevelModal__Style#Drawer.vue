  ...
  <a-drawer
    ...
    <a-spin :spinning="confirmLoading">
      <a-form :form="form">
        ...
        <a-form-item label="返佣比例" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <a-input-number v-decorator="[ 'commissionRatio', validatorRules.commissionRatio]" placeholder="请输入返佣比例" style="width: 100%"/>
        </a-form-item>
        <a-form-item label="等级描述" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <a-input v-decorator="[ 'describe', validatorRules.describe]" placeholder="请输入等级描述"></a-input>
        </a-form-item>
  ...
  export default {
    ...
    methods: {
      ...
      edit (record) {
        ...
        this.visible = true;
        this.$nextTick(() => {
          this.form.setFieldsValue(pick(this.model,'levelName','commissionRatio','describe','createTime','createBy'))
        })
      },
      close () {
        this.$emit('close');
      ...
      popupCallback(row){
        this.form.setFieldsValue(pick(row,'levelName','commissionRatio','describe','createTime','createBy'))
      }
